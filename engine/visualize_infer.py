...
def show_image_with_boxes(image, output, target, visualize_preds, vis_scores=None, depth_method=None):
	...
	# grab raw depth (C×H×W)
	raw = visualize_preds['depth_map'][0].cpu().numpy()  # shape = (1, 96, 320)

	# remove the channel dim → (96,320)
	depth_raw = np.squeeze(raw, axis=0)

	# now resize to your network’s input heatmap size (W=1280, H=384)
	depth_resized = cv2.resize(depth_raw, (1280, 384))

	# crop back to the original image
	pad_y, pad_x = pad_size[1].item(), pad_size[0].item()
	depth_map = depth_resized[pad_y : pad_y + image.shape[0], pad_x : pad_x + image.shape[1]]
    ...
    # zero-pad image_id to 6 digits
	img_id = int(target.get_field("image_id"))
	img_id_str = f"{img_id:06d}"

    # save depth TXT (meters)
	depth_txt = os.path.join(vis_dir, f"depth_{img_id_str}.txt")
	np.savetxt(depth_txt, depth_map, fmt="%.4f")

    # save heatmap (uint8)
	hm = (all_heatmap / all_heatmap.max() * 255).astype("uint8")
	cv2.imwrite(os.path.join(vis_dir, f"heatmap_{img_id_str}.png"), hm)

    # save projected 3D boxes image (convert RGB→BGR)
	img3_bgr = img3[..., ::-1]
	cv2.imwrite(os.path.join(vis_dir, f"boxes3d_{img_id_str}.png"), img3_bgr)

    # save bird’s‑eye view
	cv2.imwrite(os.path.join(vis_dir, f"bev_{img_id_str}.png"), img4)
    # ────────────────────────────────────────────────────────────────────
